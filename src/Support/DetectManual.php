 ...
 */
class DetectManual extends LaserpointsScript
{
    /**
     ...
     */
    public function execute($imagePath, $distance, $points)
    {
        $python = config('laserpoints.python');
        $script = config('laserpoints.manual_script');
        $command = "{$python} {$script} '{$imagePath}' '{$distance}' '{$points}' 2>&1";
